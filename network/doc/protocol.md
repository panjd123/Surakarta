...
5. **服务端**收到某个客户端发送的 `MOVE_OP` 或后：
    ...
    2. 如果操作非法，操作仍然会先被转发给**双方客户端**，但会被打上**标记**，随后会发送 `END_OP` 给**双方客户端**，游戏结束，可以离开
...
认输，替代 `MOVE_UP`，向服务端发送 `GIVEUP_OP`，服务端收到后不转发，而是直接进入发送 `END_OP` 的流程。

> 我们规定 `GIVE_UP` 只能在自己的回合发送，如果在对方回合发送，服务端应该忽略这个请求。
